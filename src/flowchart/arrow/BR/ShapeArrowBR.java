 ...
 * @author ZULU
 */
public class ShapeArrowBR extends BorderFlowChart {
     ...
     */
    public ShapeArrowBR(Color color) {
        super(color);
    }
...
////
        ...
        p.addPoint(x + 2 * S, y2 - S);

        p.addPoint(x + 3 * S, y2);
        p.addPoint(x, y2 - S - S / 2);
        ...
        if (shape.next instanceof Do_While) {
            String txt = Fi18N.get("DO.while");
            Dimension dimText = UtilsFlowchart.getTextDimension(txt, getLabelFont());
            drawLabel(g, txt, x + 2 * S, y2 - dimText.height );
        }
