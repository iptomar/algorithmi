 ...
 * @author 
 */
public class JavaLang extends AbstractLang {
    ...
    public String getStartOfProgram(Fshape shape) {
        ...
        return "import java.util.Scanner;\n\n"
                + "public class " + className.substring(className.lastIndexOf('\\') + 1, className.lastIndexOf('.')) + "{\n"
    ...
    public String getWrite(Fshape shape) {
        Write w = (Write) shape;
        String code = "";
        code += "System.out.println(" + w.getInstruction() + ");";
        return code;
    }
    ...
    public String getRead(Fshape shape) {
        ...
        if (r.var instanceof Freal) {
            ...
            code += r.var.getName() + " = Keyb.nextDouble();";
        } else if (r.var instanceof Finteger) {
            if (isDefined) {
                code += "int ";
            }
            code += r.var.getName() + " = Keyb.nextInteger();";
        } else if (r.var instanceof Ftext) {
            if (isDefined) {
                code += "String ";
            }
            code += r.var.getName() + " = Keyb.nextLine();";
        } else if (r.var instanceof Flogic) {
            if (isDefined) {
                code += "boolean ";
            }
            code += r.var.getName() + " = Keyb.nextBoolean();";
        } else {
            code = " ERRROR IN READ";
        }
