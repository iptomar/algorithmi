 ...
 * @author 
 */
public class CLang extends AbstractLang {

    public CLang() {
        super.IDENTATATION_SPACE=4;
        super.DEFAULT_IDENTATATION=0;
    }
    ...
    static {
        ...
        lang.put(FkeyWord.get("TYPE.real"), "double");
        lang.put(FkeyWord.get("TYPE.string"), "String");
        lang.put(FkeyWord.get("TYPE.boolean"), "boolean");
    ...
    public String getDefine(Fshape shape) {
        ...
        if(!isArray){
            ...
            code+="["+((Farray)def.varSymbol).getDimensions()+"];";
        }
        return code;
    }
    ...
    public String getWrite(Fshape shape) {
        Write w = (Write) shape;
        String code = "";
        code += "printf(" + w.getInstruction() + ");";
        return code;
    }

    public String define(Fsymbol var) {
        String code = "";
        boolean isArray=false;
        String tempArray="";
        if (var instanceof Farray){
            isArray=true;
            tempArray=getType(((Farray)var).getTemplateElement());
            code+=tempArray;
        } else {
            code+=getType(var)+" ";
        }
    ...
    public String getRead(Fshape shape) {
        ...
        String code = "";
        if (r.var instanceof Freal) {
            if (isDefined) {
                code += define(r.var)+"\n";
            }
            code += "scanf(";
            code += "\"%f\", &"+r.var.getName() + ");";
        } else if (r.var instanceof Finteger) {
            if (isDefined) {
                code += define(r.var)+"\n";
            }
            code += "scanf(";
            code += "\"%d\", &"+r.var.getName() + ");";
        } else if (r.var instanceof Ftext) {
            if (isDefined) {
                code += define(r.var)+"\n";
            }
            code += "scanf(";
            code += "\"%s\", "+r.var.getName() + ");";
        } else if (r.var instanceof Flogic) {
            if (isDefined) {
                code += define(r.var)+"\n";
            }
            code += "scanf(";
    ...
    public String getDoWhile(Fshape shape) {
        ...
        String code = "";
        code += "while(" + AbstractLang.lang.getExpression(doWhile.logicExpression) + ");";
        return code;
    }
    ...
    public String getFor(Fshape shape) {
        ...
        code += forNext.var.getName() + "=" + AbstractLang.lang.getExpression(forNext.start).trim() + "; "
             + forNext.var.getName() + "<=" + AbstractLang.lang.getExpression(forNext.stop).trim() + "; "
             + forNext.var.getName() + "=" + forNext.var.getName() + "+" + AbstractLang.lang.getExpression(forNext.step).trim() + "){";
        return code;
    }
    ...
    public String getFunction(Fshape shape) {
        ...
        String code = "";
        code += "";
        if (func.getReturnSymbol() instanceof Freal) {
            code += "double ";
        } else if (func.getReturnSymbol() instanceof Finteger) {
            code += "int ";
        } else if (func.getReturnSymbol() instanceof Ftext) {
            code += "char ";
        } else if (func.getReturnSymbol() instanceof Flogic) {
            code += "boolean ";
        } else {
            code += "void ";
        }
